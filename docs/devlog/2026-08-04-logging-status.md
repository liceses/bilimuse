...
- 验收标准：
  - `setup_logging` 生成 `<config_dir>/logs/musicalbili.log`（1MB×3 滚动），级别可配
...
- **日志**：`logging_setup.py`——根 logger `musicalbili`，文件 `RotatingFileHandler`(1MB×3) + console WARNING（用 filter 排除 `musicalbili.status`，避免与 UI echo 重复）。
...
- console handler 加 `addFilter(lambda r: not r.name.startswith("musicalbili.status"))`：emit(WARNING) 由 UI echo，不重复打控制台；内部 `logger.warning`（provider 降级）仍可见 → `logging_setup.py`
...
- `musicalbili search "晴天"` → 日志文件生成含 `INFO musicalbili.bilibili B站搜索 '晴天' -> 2 条` ✅
- `musicalbili config`（管道输入）→ 控制台显示「等待输入: 下载目录...」，日志含「已设置 download_dir = downloads / format / lyric_sources / align / sessdata / proxy」✅
- `musicalbili doctor` → 显示 `日志: .../logs/musicalbili.log（level=INFO）` ✅
