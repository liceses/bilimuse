...
| 技巧/坑 | 一句话 | 位置 |
|---|---|---|
| URL 拼接 | `f"{API}/{path.lstrip('/')}"`，API 常量忘加尾斜杠会拼成 `api.bilibili.comx` | `bilimuse/providers/bilibili.py:94` |
| Wbi 签名 | 播放接口需 wbi 签名 + buvid3 cookie 防 -412 | `bilimuse/providers/bilibili.py:76` |
...
| typer 0.27 Exit | `Exit` 只有 `code` 参数，错误信息先 `echo` 再 `Exit(code=1)` | `bilimuse/cli.py:79` |
| 下载前查 ffmpeg | 需转码时先 `shutil.which` 再下载，避免半途失败 | `bilimuse/services/download.py:87` |
| SQLite 去重 | `UNIQUE(bvid,cid)` + `INSERT OR REPLACE` 幂等 | `bilimuse/db.py:38` |
| ffmpeg 内嵌 | `imageio-ffmpeg` 可选 extra，PyPI 拉取不经 GitHub；`find_ffmpeg` config→PATH→内置 | `bilimuse/services/download.py:32` |
...
| 网易云无周杰伦版权 | 2018 授权到期，搜索只剩翻唱/remix → 咪咕 MIGUM2.0 为第一源 | `bilimuse/providers/meta.py` |
| 咪咕封面 | 用搜索结果 `imgItems` URL（勿按 id 再搜），webp 用 ffmpeg 管道转 jpeg | `meta.py:96` |
| 标题清洗/匹配 | `clean_title`+`split_query` 拆歌手/歌名，歌手命中±分 | `bilimuse/services/tagger.py:30` |
| B站扫码登录 | passport qrcode generate/poll；外层 code=0 恒真，看 `data.code`；成功从 Set-Cookie 取 SESSDATA | `bilimuse/services/auth.py` |
| B站搜索双端点 | 登录态优先 wbi，`result` 空(v_voucher)自动回退旧版 | `bilimuse/providers/bilibili.py:119` |
| 网易云 weapi | 端点用 `weapi/search/get/web`（cloudsearch 已 50000005）；v3 详情用 `al`/`ar` 键 | `bilimuse/providers/meta.py:119` |
| 登录降风控 | 匿名流量是 B 站风控重点，SESSDATA 后走 wbi 主链路 | 风险表 |
| fMP4 时长 | mutagen 读分片 MP4 时长为 0，用 ffmpeg `-i -f null -` 解析 Duration | `bilimuse/services/aligner.py:38` |
...
| LRC 清理 | 网易云剥作曲/编曲头；LRCLIB 剥空行/live标题行；翻译合并 | `bilimuse/services/lyric.py` |
| align 置信度回退 | `-f json --interpolate` 取 matched 标记；匹配率<50% 回退原歌词，不盲目缩放 | `bilimuse/services/aligner.py` |
